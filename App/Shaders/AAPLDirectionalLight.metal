...
vertex QuadInOut deferred_direction_lighting_vertex(
    constant AAPLSimpleVertex * vertices  [[ buffer(AAPLBufferIndexMeshPositions) ]],
...
half4 deferred_directional_lighting_fragment_common(
    QuadInOut in,
...
)
{
    ...
    // Calculate specular contributioh from directional light
        ...
        float4 ndc_fragment_pos = float4(
...
fragment AccumLightBuffer deferred_directional_lighting_fragment_single_pass(
...
)
{
    ...
    output.lighting = deferred_directional_lighting_fragment_common(in, frameData, GBuffer.depth,  GBuffer.normal_shadow,  GBuffer.albedo_specular);
