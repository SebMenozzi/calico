...
- (void)connectToPort:(int)port
          withManager:(USBMuxManager*)manager
    ...
    [manager connectToDevice:deviceID port:port onStart:^(NSError *err, dispatch_io_t dispatchChannel) {
        NSError *error = err;
        if (!error) {
...
- (void)connectToPort:(in_port_t)port IPv4Address:(in_addr_t)address callback:(void(^)(NSError *error, PTAddress *address))callback {
    ...
    if (connState_ != kConnStateNone) {
        if (callback)
            callback([NSError errorWithDomain:NSPOSIXErrorDomain code:EPERM userInfo:nil], nil);
        return;
    }
    connState_ = kConnStateConnecting;
    ...
    if (fd == -1) {
        ...
        error = errno;
        if (callback)
            callback([[NSError alloc] initWithDomain:NSPOSIXErrorDomain code:errno userInfo:nil], nil);
        return;
    }
    ...
    if (connect(fd, (const struct sockaddr *)&addr, addr.sin_len) == -1) {
        ...
        if (callback)
            callback([[NSError alloc] initWithDomain:NSPOSIXErrorDomain code:error userInfo:nil], nil);
    ...
    if (!dispatchChannel) {
        ...
        if (callback)
            callback([[NSError alloc] initWithDomain:@"PTError" code:0 userInfo:nil], nil);
        return;
    }
...
- (void)listenOnPort:(in_port_t)port IPv4Address:(in_addr_t)address callback:(void(^)(NSError *error))callback {
    ...
    if (fd == -1) {
        if (callback)
            callback([NSError errorWithDomain:NSPOSIXErrorDomain code:errno userInfo:nil]);
        return;
    }
...
- (NSString*)description {
    NSString *state = connState_ == kConnStateConnecting ? @"connecting" : connState_ == kConnStateConnected  ? @"connected" : connState_ == kConnStateListening  ? @"listening" : @"closed";

    return [NSString stringWithFormat:@"<PTChannel: %p (%@)>", self, state];
}
