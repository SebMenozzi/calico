...
/// https://stackoverflow.com/questions/52729878/prevent-subclassing-objective-c-classes-in-swift
#ifndef PT_FINAL
...
- (void)connectToPort:(int)port withManager:(USBMuxManager*)manager deviceID:(NSNumber *)deviceID callback:(void(^)(NSError * _Nullable error))callback NS_SWIFT_NAME(connect(to:with:deviceID:callback:));

// Connect to a TCP port at IPv4 address.
// Provided port must NOT be in network byte order.
// Provided in_addr_t must NOT be in network byte order.
...
- (void)sendFrameOfType:(uint32_t)frameType
                    tag:(uint32_t)tag
            withPayload:(nullable NSData *)payload
               callback:(nullable void(^)(NSError * _Nullable error))callback NS_SWIFT_NAME(sendFrame(type:tag:payload:callback:));
...
- (BOOL)startReadingFromConnectedChannel:(dispatch_io_t)channel
                                   error:(__autoreleasing NSError **)error NS_SWIFT_NAME(startReading(from:));
...
shouldAcceptFrameOfType:(uint32_t)type
                   tag:(uint32_t)tag
           payloadSize:(uint32_t)payloadSize NS_SWIFT_NAME(channel(_:shouldAcceptFrame:tag:payloadSize:));;

// Invoked when the channel closed. If it closed because of an error, *error* is
// a non-nil NSError object.
- (void)ioFrameChannel:(PTChannel *)channel
       didEndWithError:(nullable NSError *)error NS_SWIFT_NAME(channelDidEnd(_:error:));
