...
class Scene {
    ...
    var objectMap: [ECSEntityID: Object] = [:]
    ...
    let shadowProjectionMatrix = Float4x4.orthographicProjection(
        left: -53,
        right: 53,
        bottom: -33,
        top: 53,
    ...
    private var lightPositionsBuffers = [BufferView<Float4>]()
    
    var lightPositions: BufferView<Float4> {
        lightPositionsBuffers[currentBufferIndex]
    }
    
    // Light positions before transformation to positions in current frame
    private var originalLightPositions = [Float4]()
    ...
    // Vertex descriptor for models loaded with MetalKit
    let skyboxVertexDescriptor: MTLVertexDescriptor = {
        let descriptor = MTLVertexDescriptor()
    ...
    init(device: MTLDevice) {
        ...
        for index in 0..<maxFramesInFlight {
            ...
            let lightPositionsBuffer = BufferView<Float4>(device: device,
                                                            count: numberOfLights,
        ...
        do {
            ...
            icosahedronDescriptor.layout(AAPLVertexAttributePosition.rawValue).stride = MemoryLayout<Float4>.stride
        ...
        let sphereDescriptor = MTKModelIOVertexDescriptorFromMetal(skyboxVertexDescriptor)
        ...
        for index in 0..<fairyVerticesCount {
            ...
            let position = Float2(sinf(point * angle), cosf(point * angle))
    ...
    func populateLights() {
        ...
        for index in 0..<numberOfLights {
            ...
            let position = Float4(distance * sinf(angle), height, distance * cosf(angle), 1)
    ...
    func updateLights(frameNumber: Int, modelViewMatrix: Float4x4) {
                
        for lightIndex in 0..<numberOfLights {
            var currentPosition = Float4.zero
    ...
    func update() {
        ...
        let cameraRotationMatrix = Float4x4.rotationMatrix(angle: cameraRotationRadians, axis: cameraRotationAxis)
        
        let viewMatrix = Float4x4.lookAt(
            eye: .init(x: 0, y: 18, z: -50),
        ...
        let templeScaleMatrix = Float4x4.scaleMatrix(.init(0.1, 0.1, 0.1))
        
        let templeTranslationMatrix = Float4x4.translationMatrix(.init(0, -10, 0))
        ...
        frameData.temple_normal_matrix = Float4x4.normalMatrix(from: templeModelMatrix)
        
        let skyRotation = Float(frameNumber) * 0.005 - .pi * 0.75
        let skyRotationAxis = Float3(0, 1, 0)
        let skyModelMatrix = Float4x4.rotationMatrix(angle: skyRotation, axis: skyRotationAxis)
        frameData.sky_modelview_matrix = cameraRotationMatrix * skyModelMatrix
        
        // Update directional light color
        let sunColor = Float4(0.5, 0.5, 0.5, 1)
        frameData.sun_color = sunColor
        frameData.sun_specular_intensity = 1
        
        // Update sun direction in view space
        let sunModelPosition = Float4(-0.25, -0.5, 1, 0)
        ...
        let directionalLightUpVector = normalize((skyModelMatrix * Float4(0, 1, 1, 1)).xyz)
        let shadowViewMatrix = Float4x4.lookAt(eye: sunWorldDirection.xyz / 10, target: .zero, up: directionalLightUpVector)
        ...
        let shadowScale = Float4x4.scaleMatrix(.init(0.5, -0.5, 1))
        let shadowTranslate = Float4x4.translationMatrix(.init(0.5, 0.5, 0))
